...
# 1) Autenticação do canal quando envia qualquer mídia/mensagem como admin
async def channel_authenticate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ch and not ch.authenticated:
        ch.authenticated = True
        sess.commit()
        logger.info("Canal autenticado: %s", ch.title)
...
# 9) Mensagens de texto (criar grupo / convite) via estado
async def handle_text_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if st["state"] == "awaiting_channel_invite":
        ...
        try:
            ...
            return await update.message.reply_text("❌ Não consegui acessar o canal. Verifique se o bot é admin.")
        ...
        if not ch.authenticated:
            await update.message.reply_text(
                "❌ Canal não autenticado. Peça ao dono que envie qualquer mensagem enquanto o bot for admin."
            )
            user_states.pop(uid, None)
            return

        sess.add(GroupChannel(group_id=st["group_id"], channel_id=chat.id,
                              inviter_id=uid, accepted=None))
        sess.commit()

        group = sess.get(Group, st["group_id"])
        ...
        await ctx.bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("✅ Aceitar", callback_data=f"aceitar_{group.id}_{chat.id}"),
                InlineKeyboardButton("❌ Recusar", callback_data=f"recusar_{group.id}_{chat.id}")
            ]])
        )
        await update.message.reply_text("✅ Solicitação enviada com sucesso!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ Voltar", callback_data="menu_meus_grupos")]]))
        user_states.pop(uid, None)

# 10) Resposta ao convite
